...
The main functions we will override in the **admin.AdminSite** class are:
- **register**: to register the models to the admin site.
- **each_context**: to add extra context to the admin site.
- **_build_app_dict**: to build the app dictionary that will be used in the admin site to show the apps and models.
...
There are times when you need more than one admin site. For example, you might want to create a separate admin site for managing orders in your e-commerce application and another admin site for managing users.

and to do so you can create a new instance of the **admin.AdminSite** class and register the models to it.
...
<select onchange="window.location.href = this.value;">
    <option value="">Select a site</option>
     {% for site in admin_sites %}
         <option value="{{request.scheme}}://{{request.get_host}}/{{site.url}}" >{{site.name}}</option>
     {% endfor %}
</select>
{% endblock %}
  ```
